...
static int solve_problem(spmatrix *P, matrix *_q, spmatrix *G, matrix *h,
                               spmatrix *A, matrix *b, PyObject *opts,
                               PyObject **res) {
    ...
    */
    ...
    PyObject *key, *value;
    matrix *x, *z;
    int_t error = 0, n, i, j, p, q, nz, n_const, n_nnz, pos = 0;
    double *l_x = NULL, *u_x = NULL, *cval = NULL, *P_v = NULL;
    ...
    while (PyDict_Next(opts, &pos, &key, &value)) {
        if (PYINT_CHECK(value)){
            ...
            PyErr_SetString(PyExc_ValueError, "Option value must be integer or float number");
            error = 1;
            goto CLEAN;
        }
        if (error == 10007) {
            ...
            PyErr_SetString(PyExc_ValueError, "Unkown error when setting option");
            goto CLEAN;
        }
    }
    ...
    if ((error = GRBnewmodel(env, &mod, "KVXOPT", n, MAT_BUFD(_q), l_x, u_x,
                             NULL, NULL)))
        goto CLEAN;
    ...
    if (SP_NROWS(A)) {
        ...
        if ((error = GRBaddconstrs(mod, SP_NROWS(A), SP_NNZ(A), cbeg, cind,
...
static PyObject *qp(PyObject *self, PyObject *args, PyObject *kwargs) {
    ...
    error = solve_problem(P, q, G, h, A, b, opts, &res_grb);
